"""
这是一个关于多智能体合作场景中集中训练和分散执行 (centralized training and decentralized execution, CTDE) 策略梯度算法的 PyTorch 教程。
此教程使用了在 marl_network 中定义的 CTDEActorCriticNetwork 和在 pg 中定义的损失函数。主函数描述了使用假数据的 MARL CTDE 策略梯度算法的核心部分。
...
def maac_training_opeator() -> None:
    ...
    **maac_training_opeator 功能概述**:
    ...
    # Actor-critic 网络前向计算。
    output = model(local_state, global_state)
    # 保证 value 的形状为 $$(B, 1)$$, 这样可以使得其形状在后续计算中可以自动广播出不同智能体的维度。
    value = output.value
    # 准备用于计算策略梯度损失函数的数据。
    ...
    loss = pg_error(data)
    # 计算价值损失函数。
    # 需要注意的是，由于我们对于各个智能体都使用同一个全局的状态来计算价值函数，因此总的目标价值应当是各个客户端回报之和。
