...
def get_ind_global_state(agent_id, ally_agent_num, enemy_agent_num):
    ...
    ally_visible_feature = torch.cat([get_ally_visible_feature()for i in range(ally_agent_num - 1)])
    enemy_visible_feature = torch.cat([get_enemy_visible_feature() for i in range(enemy_agent_num)])
