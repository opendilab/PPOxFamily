...
def mapg_training_opeator() -> None:
    """
    **Overview**:
        The main function about the training process of CTDE policy gradient algorithm.
...
def maac_training_opeator() -> None:
    """
    **Overview**:
        The main function about the training process of CTDE actor-critic algorithm.
    ...
    # Actor-critic network forward propagation.
    output = model(local_state, global_state)
    # Keep value with the shape of $$(B, 1)$$, which can automatically broadcast agent dimension.
    value = output.value
    # Prepare the data for policy gradient loss calculation.
    ...
    # Note we use the same global state for all agents to calculate value, therefore the target is the
    # sum of returns of all the agents.
