...
def get_ind_global_state(agent_id, ally_agent_num, enemy_agent_num):
    ...
    ally_visible_features = []
    for i in range(ally_agent_num):
        if i != agent_id:
            ally_visible_features.append(get_ally_visible_feature())
    
    enemy_visible_feature = []
    for i in range(enemy_agent_num):
        enemy_visible_feature.append(get_enemy_visible_feature())


    return torch.cat([agent_id_feature, own_feature, *ally_visible_features, *enemy_visible_feature])
