...
def pack_data(data: List[torch.Tensor], traj_len: int) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    for traj in data:
        length, num = traj.shape
        # less than traj_len use null padding
        if length < traj_len:
            pad_traj = torch.concat([traj, torch.zeros(traj_len - length, num)])
            pad_mask = torch.ones(traj_len)
            pad_mask[length:] = 0.0
            traj_list.append(pad_traj)
            mask_list.append(pad_mask)
        # greater than traj_len use split and forward padding
        else:
            for i in range(0, length, traj_len):
                if i+traj_len <= length:
                    pad_traj = traj[i:i+traj_len]
                else:
                    pad_traj = traj[-traj_len:]
                traj_list.append(pad_traj)
                mask_list.append(torch.ones(traj_len))

    traj_pad_and_split = torch.stack(traj_list, dim=1)
    mask_pad_and_split = torch.stack(mask_list, dim=1)

    return traj_pad_and_split, mask_pad_and_split
