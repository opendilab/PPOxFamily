...
class PPOFModel(nn.Module):
    ...
    def init_weights(self) -> None:
        ...
        # Orthogonal initialization with small initial weights for critic
        for module in self.critic.modules():
            if isinstance(module, nn.Linear):
                init.orthogonal_(module.weight, gain=0.01)

        # Xavier initialization for actor
        for module in self.actor.modules():
            if isinstance(module, nn.Linear):
                init.xavier_uniform_(module.weight)

        # Xavier initialization with small initial weights for mu
        init.xavier_uniform_(self.mu.weight, gain=0.01)

        # Normal distribution initialization for log_sigma
        init.normal_(self.log_sigma, mean=-3, std=0.1)
