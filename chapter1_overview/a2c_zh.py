...
def a2c_error(data: namedtuple) -> namedtuple:
    ...
    **概述**:
    ...
    # 策略模型的损失: $$- \frac 1 N \sum_{n=1}^{N} log(\pi(a^n|s^n)) A^{\pi}(s^n, a^n)$$
    policy_loss = -(logp * adv * weight).mean()
    # 值模型的损失: $$\frac 1 N \sum_{n=1}^{N} (G_t^n - V(s^n))^2$$
    ...
    # 注意：最终的损失是 ``policy_loss + value_weight * value_loss - entropy_weight * entropy_loss``
    entropy_loss = (dist.entropy() * weight).mean()
    # Return the concrete loss items.
    # 返回最终的各项损失信息：包含策略模型的损失，值模型的损失和熵损失。
