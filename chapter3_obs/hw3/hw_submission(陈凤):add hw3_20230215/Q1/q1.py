...
class MLP(nn.Module):
    ...
    def forward(self, x):
        if self.use_cos:
            hidden = (self.fc1(torch.cos(x)))
        else:
            hidden = self.fc2(self.relu_layer(self.fc1(x)))
        y_pred = self.fc3(hidden)
        return y_pred
...
def alg1(x_train, y_train_true, x_test, y_test_true):
...
def alg2(x_train, y_train_true, x_test, y_test_true):
...
def alg3(x_train,y_train_true,x_test,y_test_true):
...
if __name__ == "__main__":
    ...
    x_test = torch.randint(0, 1000, size=(500, 1)).float()
    y_test_true = x_test % 2



    # 选择一个方案测试效果
    # alg1(x_train,y_train_true,x_test,y_test_true)
    # alg2(x_train, y_train_true,x_test, y_test_true)
    alg3(x_train,y_train_true,x_test,y_test_true)